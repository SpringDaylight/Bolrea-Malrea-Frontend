...
export const syncUserPreferenceToLocal = async (userId: string): Promise<boolean> => {
  try {
    ...
    // Check if new nested "global" structure or old flat structure
    const isNested = preference_vector_json && "global" in preference_vector_json;
    const baseScores = isNested
      ? (preference_vector_json as any).global
      : preference_vector_json;

    // 감정 태그를 텍스트로 변환 (상위 3개)
    const emotionScores = baseScores?.emotion_scores || {};
    const topEmotions = Object.entries(emotionScores)
      .sort(([_, a], [__, b]) => (b as number) - (a as number))
      .slice(0, 3)
      .map(([tag, _]) => tag);

    const narrativeTraits = baseScores?.narrative_traits || {};
    const topNarratives = Object.entries(narrativeTraits)
      .sort(([_, a], [__, b]) => (b as number) - (a as number))
      .slice(0, 3)
      .map(([tag, _]) => tag);
